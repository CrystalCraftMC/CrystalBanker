            ...
            if(player.hasPermission("crystalbanker-transaction")){
                ...
                if(cmd.getName().equalsIgnoreCase("crystalbanker") && args.length == 2 && args[1].equalsIgnoreCase("deposit") && isInt(args[2])){
                    ...
                    if(countXP(player)){
                        ...
                        int xpTally = 0;
                        if(currentLevel <= 16) xpToBottles(player, formulaTierOne(player, storeLevels, currentLevel, clXP, xpTally));
                        else if (currentLevel <= 31) xpToBottles(player, forumlaTierTwo(player, storeLevels, currentLevel, clXP, xpTally));
                        else if (currentLevel >= 32) xpToBottles(player, forumlaTierThree(player, storeLevels, currentLevel, clXP, xpTally));
                        return true;
                    } else return false;
                }
...
//Add levels
        ...
        if(leftToRemove == 0){
            ...
            for (int i = 0; i < amountToRemove; i++){//translate everything into orbs
                ...
                Bukkit.broadcastMessage(ChatColor.DARK_RED + "Debug: Values of Orbs being output: " + xpOrb);
                xpOrbTotal += xpOrb;
            }
...
//changes xp to bottles which can stored in chests until used
    public static void xpToBottles(Player player, int totalXPToChange){
        ...
        int currentXP = player.getTotalExperience();
        if(countXP(player) && currentXP > totalXPToChange){
            float resultXP = 0;
